Room nights are `sum(number_of_spaces)` at the stay-date row grain, so one reservation can produce multiple stay rows and multiple room nights. Stay rows are individual `reservation_id x stay_date` rows. Reservations are `count(distinct reservation_id)` in the filtered universe.
